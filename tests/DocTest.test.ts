...
test('DocTest removes hidden section markers (###...###) from code', () => {
    const test = new DocTest(' * # foo(###1###);\n * # bar(###"string"###);', 0, 0, 'Mock.Test', errorResolver());
...
// Generation -----------------------------------------------------------------
test('DocTest does include the line number and column in the generated test', () => {
    const test = new DocTest(' * foo();\n * bar();', 31, 41, 'Mock.Test', errorResolver());
    expect(test.generate()).toEqual({
        name: 'Mock.Test.line-32.col-42',
        source: '// Auto generated doc test\n\n\ntest(\'Mock.Test (line 32, column 42)\', () => {\n    foo();\n    bar();\n});\n\n'
    });
});
...
test('DocTest rewrites dynamic import(...) calls to their new relative targets', () => {
    const test = new DocTest(' * const Bar = import("./bar/Bar"); const Foo = import("./Foo");', 0, 0, 'Mock.Test', importResolver({
        './Foo': 'src/Foo',
        './bar/Bar': 'src/bar/Bar',
    }));
...
test('DocTest ignore empty import(...) calls', () => {
    const test = new DocTest(' * const Empty = import();', 0, 0, 'Mock.Test', errorResolver());
    expect(test.generate()).toEqual({
        name: 'Mock.Test.line-1.col-1',
        source: '// Auto generated doc test\n\n\ntest(\'Mock.Test (line 1, column 1)\', () => {\n    const Empty = import();\n});\n\n'
    });
